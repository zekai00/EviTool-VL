...
后续将 clipped + reference KL 扩到同一 SFT v3 起点的 fair 289 trainable groups，并补跑同数据 vanilla 对照。两者在 `val_balanced_70` 上均为 `0.855`，高于 SFT v3 baseline，但低于 60tg clipped+KL probe 和历史 staged 289tg 主线，因此都未升级为 current model。这个结果说明 one-shot 扩大 trainable groups 不足以替代 staged on-policy curriculum。
...
A later same-start fair 289 trainable-group probe trained both vanilla and clipped + reference KL variants. Both reached `0.855` on `val_balanced_70`, above the SFT v3 baseline but below the 60tg clipped+KL probe and the historical staged 289tg mainline, so neither was promoted to current model. This suggests that one-shot scaling of trainable groups is not a substitute for staged on-policy curriculum.
