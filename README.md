...
同一 SFT v3 起点、同一批 60 trainable groups 的 step-wise loss 形式对比：
...
这里的加粗只表示 clipped + reference KL 在“同起点、同数据量”的 60tg 消融中最好；它还不是当前主线最佳模型。当前主线 staged 289tg GRPO 使用分阶段累计 on-policy groups，在同一评测上达到 `val balanced=0.942`、`val200=0.910`。

后续将 clipped + reference KL 扩到同一 SFT v3 起点的 fair 289 trainable groups，并完成 clipped+KL 训练。该 adapter 在 `val_balanced_70` 上达到 `0.855`，高于 SFT v3 baseline，但低于 60tg clipped+KL probe 和历史 staged 289tg 主线，因此未升级为 current model。对应 vanilla 289tg 对照组因 GPU/NVML 异常没有得到可用 adapter，后续需在 GPU 状态恢复后补跑。
...
Same SFT v3 starting point and same 60 trainable groups:
...
The bold numbers only mark the best method in the same-start, same-data 60tg ablation. The current best mainline model is still staged 289tg GRPO, which uses accumulated on-policy groups and reaches `val balanced=0.942`, `val200=0.910`.

A later same-start fair 289 trainable-group probe trained the clipped + reference KL variant and reached `0.855` on `val_balanced_70`. This is above the SFT v3 baseline but below the 60tg clipped+KL probe and the historical staged 289tg mainline, so it was not promoted to current model. The matching vanilla 289tg control did not produce a usable adapter because of a GPU/NVML failure and should be rerun after the GPU state is healthy.
